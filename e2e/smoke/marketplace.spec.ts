...
import { openCoverageAssistant, selectRadixOption } from "../helpers/marketplace";
...
test.describe("authenticated app smoke", () => {
  ...
  test("marketplace filtering updates results", async ({ page }) => {
    ...
    await expect(page.getByRole("region", { name: "Filtros de búsqueda" })).toBeVisible();

    await page.getByLabel("Edad").fill("30");
    await selectRadixOption(page, "Género", "Femenino");
    await selectRadixOption(page, "Región", "Nacional");
    await page.getByRole("button", { name: "Aplicar filtros" }).click();

    await expect(page).toHaveURL(/age=30/);
    await expect(page.getByText("Plan Básico Alpha").first()).toBeVisible();
  });
  ...
  test("plan detail shows policy viewer and assistant", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Plan Básico Alpha" }),
    ).toBeVisible();
